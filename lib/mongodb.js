...
MongoDB.prototype.execute = function(model, command) {
  ...
  var params = args.length > 1 ? args[0] : [];
  ...
  this.notifyObserversAround('execute', context, function(context, done) {
    ...
    debug('MongoDB: model=%s command=%s', model, command, params);
    return collection[command].apply(collection, params);
  }, callback);
};
...
MongoDB.prototype.replaceWithOptions = function(model, id, data, options, cb) {
  var self = this;
  this.execute(model, 'update', [{ _id: id }, data, options], options, function(err, info) {
